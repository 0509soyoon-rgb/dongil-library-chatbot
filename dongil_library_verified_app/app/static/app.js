...
function hideOptionalPanels(){
  const librarianPanel = $('#librarianPanel');
  const typeTest = $('#typeTest');
  if(librarianPanel) librarianPanel.classList.add('hidden');
  if(typeTest) typeTest.classList.add('hidden');
}
...
$('#typeBtn').addEventListener('click', () => { clearMainResults(); const librarianPanel = $('#librarianPanel'); if(librarianPanel) librarianPanel.classList.add('hidden'); $('#typeTest').classList.toggle('hidden'); $('#typeTest').scrollIntoView({behavior:'smooth'}); });
